 ...
 * produits du magasin.
 * En d'autres termes, cette classe est utilisée pour initialiser votre magasin dès le démarrage de l'application. En application réelle, les éléments seront extraits de la session utilisateur ou de la base de données. Pour plus de simplicité,  on utilise la classe Produits pour initialiser des objets.
 * 
 * Créer une méthode getProducts pour retourner une liste de type Produit.
 * 
 * Créer une méthode initStoreItems () qui vous permet d’ajouter les produits dans une nouvelle ArrayList de type Produit.
 * @author vanessa
 */
